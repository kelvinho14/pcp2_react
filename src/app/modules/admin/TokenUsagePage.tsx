...
const TokenUsagePage: FC = () => {
  ...
  useEffect(() => {
    ...
    if (selectedPurpose) {
      const purposeKey = Object.keys(PURPOSE_MAPPING).find(key => PURPOSE_MAPPING[Number(key)] === selectedPurpose)
  ...
  const columns = useMemo<Column<TokenUsageRecord>[]>(() => [
    ...
    },
    {
      ...
      Cell: ({ value }) => {
        ...
        const purposeKey = Object.keys(PURPOSE_MAPPING).find(key => PURPOSE_MAPPING[Number(key)] === value)
        const displayValue = purposeKey ? PURPOSE_MAPPING[Number(purposeKey)] : value
